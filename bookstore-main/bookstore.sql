...
CREATE TABLE `address` (
  ...
  CONSTRAINT `FK_address_fu` FOREIGN KEY (`uid`) REFERENCES `consumer` (`uid`)
...
CREATE TABLE `cart` (
  ...
  CONSTRAINT `FK_cart_fu` FOREIGN KEY (`uid`) REFERENCES `consumer` (`uid`)
...
CREATE TABLE `orders` (
  ...
  KEY `uid` (`uid`),
  KEY `aid` (`aid`),
  CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`uid`) REFERENCES `consumer` (`uid`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `relate` (
  ...
  CONSTRAINT `FK_relate_ru` FOREIGN KEY (`uid`) REFERENCES `consumer` (`uid`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
